...
class Predictor():
    ...
    def predict(self, img):
        ...
        if self.config['predictor']['beamsearch']:
            ...
            s = translate(img, self.model)
            s = s[0].tolist()
        s = self.vocab.decode(s)
        return s

    def batch_predict(self, images, set_bucket_thresh = 40):
        """
    ...
    def batch_process(self, images, set_bucket_thresh):
        ...
        for i, image in enumerate(img_li):
            ...
            # save_img = np.transpose(new_img * 255.0, (1, 2, 0))
            # print(new_img.shape)
            # cv2.imwrite(f"images_vietocr/{str(avg_bucket_width)}_{str(i)}.jpg", save_img)
